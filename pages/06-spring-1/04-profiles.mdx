...
1. Dodaj jeszcze jedną klasę w tym samym pakiecie: `GreenMailHandler`. Wstrzyknij do niej obiekty typu `GreenMail` i wystartuj usługę: `greenMail.start()`. Dodaj metodę, która powinna wywołać się podczas zamykania systemu i wypisywać zgromadzone maile, wysłane podczas sesji:
    ```java
    @PreDestroy
