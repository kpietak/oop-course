...
1. W klasie `Student` zaimplementuj metodę `create`. Powinna ona dodawać rekord do tabeli `student`. Aby zaimplementować tę metodę musisz (można spojrzeć na metodę `create` klasy `Course`):
    ...
    * Pobrać stworzonego studenta za pomocą metody `Student.findById()`.
        ...
        Po wykonaniu tego zadania powinien działać test `createStudentTest()` z klasy `ActiveRecordTest`. Sprawdź to uruchamiając ten test.
2. W klasie `Student` zaimplementuj metodę `findByIndexNumber()`. Skorzystaj z `QueryExecutor.read()`. Aby to zrobić musisz:
    ...
    * Na podstawie pobranych danych utworzyć nowy obiekt klasy `Student`.
        ...
        Po wykonaniu tego zadania powinien działać test `findStudentIndexTest()` z klasy `ActiveRecordTest`. Sprawdź to uruchamiając ten test.
...
Po wykonaniu tego zadania powinien działać test `enrollStudentTest()` z klasy `ActiveRecordTest`. Sprawdź to uruchamiając ten test.
...
Sprawdź poprawność wykonania tego zadania uruchamiając test `courseStudentListTest()` z klasy `ActiveRecordTest`.
...
## Usprawnienie pobierania listy studentów (do zrobienia jeśli jest dużo czasu)
...
Sprawdź poprawność wykonania tego zadania uruchamiając test `cachedCurseStudentListTest()` z klasy `ActiveRecordTest`.
...
Sprawdź poprawność wykonania tego zadania uruchamiając test `gradeStudentTest()` z klasy `ActiveRecordTest`.
