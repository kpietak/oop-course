...
2. Część zadań będzie wymagała użycia wbudowanego scrapera do pobierania obrazów z Google Grafika. Korzystamy z **Serper API**, które wymaga podania indywidualnego wygenerowanego klucza API. Wejdź na [stronę Serper API](https://serper.dev) i załóż tam konto. Następnie w sekcji *API Keys* znajdź swój domyślny klucz, skopiuj go i wklej w klasie `CrawlerApp` (stała `SERPER_API_KEY`).

	<Callout type="warning">
    **Uwaga:** w darmowej wersji Serper API umożliwia wykonanie 2500 zapytań. Na potrzeby tej i kolejnej laborki w zupełności powinno wystarczyć, ale warto o tym pamiętać. Możesz sprawdzić aktualną wartość wykorzystanego limitu w panelu Billing. Serper nie wymaga podawania danych karty płatniczej przy rejestracji!
	</Callout>
