...
* `public Session getCurrentSession()` - metoda pozwalająca na stworzenie obiektu Session, który będziemy wykorzystywać następnie do tworzenia zapytań do bazy danych.
...
1. Zaimplementuj metodę `findByIndexNumber()` w klasie `StudentDao`. Użyj do tego metody `getCurrentSession().createQuery()`. Pamiętaj o złapaniu wyjątku PersistencException.  
